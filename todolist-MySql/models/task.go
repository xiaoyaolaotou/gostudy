...
func GetTaskById(id int) (Task, error) {
	...
	row.Scan(&task.Id, &task.Name, &task.Progress, &task.User, &task.Desc, &task.Status, &task.CreateTime, &task.CompleteTime)
...
func ModifyTask(id int, name, desc string, progress int, user string, status int) {
	...
	_, err = db.Exec("update todolist_task set name=?,`desc`=?,progress=?,user=?,status=? where id=?", name, desc, progress, user, status, id)
	if err != nil {
		panic(err)
	}
